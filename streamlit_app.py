...
# إعدادات الصفحة
st.set_page_config(page_title="منصة الاختبارات المقالية", layout="wide")

# كود التطبيق التفاعلي (HTML + CSS + JS)
html_code = """
<!DOCTYPE html>
...
<head>
    ...
    <style>
        * { box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        body { background-color: #f0f4f8; margin: 0; padding: 10px; display: flex; justify-content: center; }
        .quiz-container { background: #ffffff; width: 100%; max-width: 900px; border-radius: 16px; box-shadow: 0 8px 24px rgba(0,0,0,0.1); padding: 20px; border: 2px solid #e1e8ed; }
        .header { text-align: center; border-bottom: 3px solid #2563eb; padding-bottom: 15px; margin-bottom: 20px; }
        .header h1 { color: #1e293b; margin: 0 0 5px 0; font-size: 22px; }
        .header p { color: #64748b; margin: 0; font-size: 14px; }
        .controls { display: flex; flex-wrap: wrap; gap: 10px; justify-content: space-between; align-items: center; margin-bottom: 20px; background: #eff6ff; padding: 12px; border-radius: 10px; }
        .btn-action { background-color: #059669; color: white; border: none; padding: 10px 16px; font-size: 14px; font-weight: bold; border-radius: 8px; cursor: pointer; }
        .btn-action:hover { background-color: #047857; }
        .btn-blue { background-color: #2563eb; }
        .btn-blue:hover { background-color: #1d4ed8; }
        .question-card { background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 12px; padding: 18px; margin-bottom: 18px; }
        .question-title { font-size: 16px; font-weight: bold; color: #0f172a; margin-bottom: 10px; }
        ...
        textarea { width: 100%; height: 80px; padding: 10px; border: 2px solid #cbd5e1; border-radius: 8px; font-size: 14px; resize: vertical; margin-bottom: 10px; }
        textarea:focus { border-color: #2563eb; outline: none; }
        .feedback-box { display: none; background-color: #f0fdf4; border-right: 4px solid #16a34a; padding: 12px; border-radius: 6px; font-size: 14px; color: #14532d; margin-top: 10px; line-height: 1.6; }
        .action-container { text-align: center; margin-top: 25px; }
        .submit-btn { background-color: #2563eb; color: white; border: none; padding: 12px 30px; font-size: 17px; font-weight: bold; border-radius: 10px; cursor: pointer; box-shadow: 0 4px 12px rgba(37, 99, 235, 0.3); }
...
const masterQuestionBank = [
    ...
    { type: "بم تفسر", q: "تلوث الهواء تؤثر سلباً على صحة الإنسان؟", modelAnswer: "لأنه يؤدي لاستنشاق غازات سامة وأتربة تسبب أمراض الجهاز التنفسي وحالات الخنق." },
...
function generateExam(count) {
    let selected = count >= masterQuestionBank.length ? [...masterQuestionBank] : [...masterQuestionBank].sort(() => 0.5 - Math.random()).slice(0, count);
    ...
    currentQuestions.forEach((item, index) => {
        ...
        card.innerHTML = `
            ...
            <div class="feedback-box" id="feed-${index}">
                ...
                ${item.modelAnswer}<br>
                <span style="color: #047857; font-weight: bold; display: block; margin-top: 5px;">🌟 ممتاز! قارن إجابتك بالإجابة النموذجية لضمان الدرجة النهائية.</span>
            </div>
        `;
...
function correctExam() {
    ...
    alert("تم عرض الإجابات النموذجية بنجاح! قارن إجاباتك بالإجابة الأدق.");
}
...
# تشغيل الواجهة التفاعلية داخل Streamlit
